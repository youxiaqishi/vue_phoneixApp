<template>
  <div class="page refundSalesPage">
    ...
    <div class="page-content">
      ...
      <div class="goodsList">
        <dl class="infoData">
          <dt><img v-bind:src="data.orderCommodityInfoDTOS[$route.params.index].url"/></dt>
          <dd>
            <h3>{{data.orderCommodityInfoDTOS[$route.params.index].commodityName}}<br /></h3>
            <div class="price">
              <p class="owner" v-if="data.orderCommodityInfoDTOS[$route.params.index].activityInfoDTO.type == 'COMMON'">业主价：<span>￥{{data.orderCommodityInfoDTOS[$route.params.index].privilegePrice | twoDecimal}}</span></p>
              <p class="owner" v-if="data.orderCommodityInfoDTOS[$route.params.index].activityInfoDTO.type == 'SECKILL'">秒杀价：<span>￥{{data.orderCommodityInfoDTOS[$route.params.index].activityInfoDTO.activityPrice | twoDecimal}}</span></p>
              <p class="owner" v-if="data.orderCommodityInfoDTOS[$route.params.index].activityInfoDTO.type == 'FLASH_SALE'">限时价：<span>￥{{data.orderCommodityInfoDTOS[$route.params.index].activityInfoDTO.activityPrice | twoDecimal}}</span></p>
              <p class="owner" v-if="data.orderCommodityInfoDTOS[$route.params.index].activityInfoDTO.type == 'GROUP'">拼团价：<span>￥{{data.orderCommodityInfoDTOS[$route.params.index].activityInfoDTO.activityPrice | twoDecimal}}</span></p>
              <p class="selling">零售价：<span>￥{{data.orderCommodityInfoDTOS[$route.params.index].originPrice | twoDecimal}}</span></p>
            </div>
            <div class="nums">x {{data.orderCommodityInfoDTOS[$route.params.index].count}}</div>
          </dd>
        </dl>
      </div>
...
export default{
  ...
  methods: {
    getData () {
      ...
      that.money = parseFloat(that.data.orderCommodityInfoDTOS[that.$route.params.index].totalPrice)
      that.refundMoneyTotal = parseFloat(that.data.orderCommodityInfoDTOS[that.$route.params.index].totalPrice)
      // 更新: 如果是最后一个商品的退款 退款最大金额加上运费
      var test = true
      that.data.orderCommodityInfoDTOS.forEach(function (item, index) {
        if (index != that.$route.params.index) {
          if (item.commodityRefundStatus === 'NONE') test = false
        }
      })
      if (test) {
        that.money += parseFloat(that.data.freight)
        that.refundMoneyTotal += parseFloat(that.data.freight)
      }
    },
    select () { // 弹出选择原因
      this.showCause = true
    ...
    submit: _.debounce(function () {
      ...
      var params = {
        ...
        'goodsId': [that.data.orderCommodityInfoDTOS[that.$route.params.index].orderCommodityId]
      }
      console.log(params)
      if (!that.submitCause) {
      ...
      that.$axios.post('/order/app/refundMoney', params).then((res) => {
        if (res) { // 更改商品审核状态
          that.data.orderCommodityInfoDTOS[that.$route.params.index].commodityRefundStatus = 'AUDIT'
          // 检测是否所有商品都已经退款 如果是 回到订单列表
          var test = true
          that.data.orderCommodityInfoDTOS.forEach(function (item) {
          ...
          that.$loading.close()
          test ? window.history.go(-2) : window.history.go(-1)
        }
      })
    }, 1000, {
      'leading': true,
