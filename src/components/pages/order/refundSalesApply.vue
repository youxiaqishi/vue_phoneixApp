<template>
  <div class="page refundSalesPage">
    ...
    <div class="page-content">
      ...
      <div class="goodsList">
        <dl class="infoData" v-for="(item, index) in data.orderCommodityInfoDTOS">
          ...
          <i class="choose" :class="{'icon-checked':!item.choose,'icon-check':item.choose}"  v-on:click="chooseRefund(item)" v-if="$route.params.type == 1"></i>
        </dl>
      </div>
      ...
      <div class="inputDiv" v-if="$route.params.type == 1">
        <div class="title">
          上传凭证
        </div>
        <ul class="imgList">
...
export default{
  ...
  methods: {
    getData () {
      ...
      if (that.$route.params.type === '0') { // 申请退款:所有未退款金额
        // UPDATE: 申请退款包含运费
        // 申请退款时 最多退款金额为所有未退款金额+运费
        that.chooseMoney = that.data.freight
        that.refundMoneyTotalPrice = that.data.freight
      ...
      if (that.$route.params.type === '1') { // 申请退货默认不选择商品退款金额为0
        that.title = '申请退货'
        that.refundReason = '退货原因'
        that.money = 0
        that.data.orderCommodityInfoDTOS.forEach(function (item) { // 选择
          that.$set(item, 'choose', false)
        })
      }
    },
    chooseRefund (info) { // 勾选(申请退货会触发)
      var that = this
      info.choose = !info.choose
      that.chooseMoney = 0.00
      that.data.orderCommodityInfoDTOS.forEach(function (item) { // 计算勾选金额
      ...
      if (that.submitCause === 'QUALITY') that.money = that.chooseMoney + parseFloat(that.data.freight)
      that.money = that.money.toFixed(2)
    },
    singleRefund (index) { // 单个商品申请退款
      this.$router.push({path: '/refundSalesApplySingle/' + index})
    },
    select () { // 弹出选择原因
      this.showCause = true
    },
    ...
    confirm () { // 确定原因
      ...
      if (this.$route.params.type === '1') { // 申请退货是质量原因退还运费
        if (this.submitCause === 'QUALITY' && !this.isUseFreight) {
          this.money = parseFloat(this.chooseMoney) + parseFloat(this.data.freight)
